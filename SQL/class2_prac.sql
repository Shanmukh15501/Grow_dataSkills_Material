...
#SELF
SELECT * FROM noob_db.employees as emp join noob_db.employees as manager  on emp.EMPLOYEE_ID=manager.MANAGER_ID


CREATE TABLE IF NOT EXISTS payment (payment_amount decimal(8,2), 
payment_date date, 
store_id int);
...
SELECT store_id,SUM(payment_amount),year(payment_date) as year FROM payment group by year,store_id order by year ;
